...
// Routes through /api/track-click so we can see who actually opens the testimonial —
// it tags + notes the GHL contact on click, then redirects to /testimonials/.
function trackedTestimonialsUrl(contactId) {
  return `bookedclinics.ca/api/track-click?c=${contactId}`;
}
...
const MESSAGES = {
  1: (name, contactId) =>
    `Hey ${firstName(name)} — one of our clients just did 500+ patient inquiries and 70 new patients in his first 60 days with us. Hear it from him directly: ${trackedTestimonialsUrl(contactId)}`,

  2: (name, contactId) =>
    `Hey ${firstName(name)}, sharing this in case you missed it — Dr. Ziani generated 500+ inquiries, 120 consultations, and 70 new patients in his first 60 days. ${trackedTestimonialsUrl(contactId)} — if you want to see what that could look like for your clinic, grab a free call here: ${CALENDAR_URL}`,
...
async function run() {
  ...
  for (const t of targets) {
    ...
    console.log(`     → "${MESSAGES[msgNum](t.name, t.contactId)}"\n`);
  }
  ...
  for (const t of targets) {
    const result = await sendSms(t.contactId, MESSAGES[msgNum](t.name, t.contactId));
    if (result.ok) {
