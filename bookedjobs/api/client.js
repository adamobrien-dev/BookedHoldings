...
module.exports = (req, res) => {
  ...
  // Resolve the client HTML file (files bundled via includeFiles in vercel.json)
  const htmlPath = path.join(__dirname, '..', 'clients', subdomain, 'index.html');

  try {
    if (!fs.existsSync(htmlPath)) {
      // Try the custom 404 page
      const notFoundPath = path.join(__dirname, '..', 'clients', '404.html');
